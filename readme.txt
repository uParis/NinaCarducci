Performances (75/100)
- (Les éléments d'image ne possèdent pas de width ni de height explicites) | Ajout de width & height explicites

- (Diffusez des images aux formats nouvelle génération) | Conversion des images PNG en WebP
- (Élément identifié comme "Largest Contentful Paint") |
- (Activez la compression de texte) |
- (Éliminez les ressources qui bloquent le rendu) |
- (Réduisez les ressources CSS inutilisées) |
- (Réduisez la taille des ressources CSS) |
- (Réduisez la taille des ressources JavaScript) |
- (Réduisez les ressources JavaScript inutilisées) |
- (La page a empêché la restauration du cache amélioré) |
- (Évitez d'énormes charges utiles de réseau) |

Accessibilité (100/100)
- (Des éléments d'image n'ont pas d'attribut [alt]) | Ajout des [alt] aux images vides
- (Le document ne contient pas d'élément <title>) | Ajout d'un titre a la page
- (Les éléments de formulaire ne sont pas associés à des libellés) | Ajout de libellé
- (Les liens n'ont pas de nom visible) | Ajout d'un [alt] au lien
Contraste ?
- (L'élément <html> n'a pas d'attribut [lang]) | Ajout d'un [lang](fr)
- (Les éléments d'en-tête ne sont pas classés séquentiellement par ordre décroissant) | Modification h6 -> p  &  h3 -> h2
- (Les couleurs d'arrière-plan et de premier plan sont suffisamment contrastées) | Exécuter style.css avant bootstrap.css

Bonnes pratiques (100/100)
- (Images affichées dans un format incorrect) | Ajouts du style height

SEO (100/100)
- (Le document ne contient pas d'élément <title>) | Ajout d'un titre a la page
- (Le document ne contient pas d'attribut "meta description") | Ajout d'un attribut "meta description"
- (Des éléments d'image n'ont pas d'attribut [alt]) | Ajout des [alt] aux images vides